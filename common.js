...
var cart = {
  'add_mod': function(product_id, quantity, option, option_qty) {
    console.log(option.length);

    if (option.length != 0) {
      var opt = '&' + option.attr('name') + '=' + option.val();
    } else {
      var opt = '';
    }
    ...
    $.ajax({
      ...
      type: 'post',
      data: 'product_id=' + product_id + '&quantity=' + (typeof (quantity) != 'undefined' ? quantity : 1) + opt + '&option_qty=' + (typeof (option_qty) != 'undefined' ? option_qty : 1),
      dataType: 'json',
      cache: false,
      beforeSend: function() {
      ...
      success: function(json) {
        ...
        if (json['error']) {
          ...
          // }
          // if (json['error']['option_quantity']) {
          //     rmNotify('danger', '<div class="text-danger">' + json['error']['option_quantity'] + '</div>');
  ...
  'add': function(product_id, quantity, option, option_qty) {
    console.log(option.length);
