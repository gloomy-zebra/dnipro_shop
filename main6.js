...
$(function() {
  ...
  $(document).on("click", ".rm-plus", function() {
    ...
    const max_qty = $(this).parent().parent().find('.rm-radio.selected .input-option_qty').val();
  ...
  // РїСЂРѕРІРµСЂРєР° РЅР° РґРѕСЃС‚СѓРїРЅРѕРµ РєРѕР»РёС‡РµСЃС‚РІРѕ С‚РѕРІР°СЂР° РѕРїС†РёРё
  $(document).on('click', ".rm-radio", function() {
    updateMaxValueProduct($(this));

  });

  function updateMaxValueProduct(_this) {
    ...
    if (Number(input_quantity) > Number(max_qty)) {
      _this.parent().parent().parent().parent().parent().parent().next().find('#input-quantity').val(max_qty);
    }

    if (input_quantity == 0 && max_qty > 0) {
      _this.parent().parent().parent().parent().parent().parent().next().find('#input-quantity').val(1);
    }

  };

  // РѕР±РЅРѕРІР»РµРЅРёРµ С†РµРЅС‹ РїСЂРё РІС‹Р±РѕСЂРµ РєРѕР»РёС‡РµСЃС‚РІР°
  $(document).on('click', '.rm-minus', function() {
    updateProductPrice3($(this));
  });
  ...
  function updateProductPrice3(_this, onen) {

    var product = _this.parent().parent().find('.rm-radio.selected').find('input').data('product-id').toString();
    var qty = _this.parent().find('input').val();
    var opt = _this.parent().parent().find('.rm-radio.selected').find('input').attr('name').toString();
    var opt_val = _this.parent().parent().find('.rm-radio.selected').find('input').val();

    $.ajax({
      type: 'post',
      url: 'index.php?route=octemplates/main/oct_functions/updatePrices',
      data: { product_id: product, quantity: qty, [opt]: opt_val },
      dataType: 'json',
      cache: false,
      success: function(json) {

        if (json['special']) {
          _this.parent().parent().find('.rm-module-price-old').html(json['price']);
          _this.parent().parent().find('.rm-module-price').html(json['special']);
        } else {
          _this.parent().parent().find('.rm-module-price').html(json['price']);
        }

        if (json['you_save']) {
          _this.parent().parent().find('.rm-module-sticker-discount').html(json['you_save']);
        }
      }
    });
  }
  ...
  $(document).on('click', '.rm-cat-button-cart', function() {
    const productID = $(this).parent().parent().parent().next().find('input[name="product_id"]').val();
    const quantity = $(this).parent().parent().parent().next().find('.form-control').val();
    var option = $(this).parent().parent().parent().parent().find('.rm-radio.selected .input-radio');
    var option_qty = $(this).parent().parent().parent().parent().find('.rm-radio.selected input[name="option_qty"]').val();
    cart.add(productID, quantity, option, option_qty);
  });

  // add cart for module
  $(document).on('click', '.rm-mod-button-cart', function() {
    ...
    var option = $(this).parent().parent().parent().parent().find('.rm-radio.selected .input-radio');
    var option_qty = $(this).parent().parent().parent().parent().find('.rm-radio.selected input[name="option_qty"]').val();
    cart.add_mod(productID, quantity, option, option_qty);
  });
